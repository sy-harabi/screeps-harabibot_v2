...
function decode_array(this: NormalizedConfig, str: string, meta: MetaInfo): [number[], number] {
  assert(
    !this.meta || (meta.depth as number) > 0 || ((meta.depth as number) === 0 && Array.isArray(this.depth)),
    TypeCodecError,
    "Array decoding error (check inputs and codec config)",
  )
  ...
  while (i < length) {
    ...
    if (value_done > 0) values[i++] = value_acc
  }
...
export class Codec {
  ...
  encode(arg: number | number[] | TypedArray): string {
    assert(
      (Array.isArray(arg) || !!(arg as TypedArray).BYTES_PER_ELEMENT) === this.array,
      TypeCodecError,
      "Incompatible codec (array <=> single value), arg =",
      arg,
    )
