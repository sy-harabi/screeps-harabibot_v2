...
function buildResourceTargets(sources: readonly Source[], minerals: readonly Mineral[]): ResourceTarget[] {
  return [
...
function findResourceEndpointPlan(
  ...
  for (let index = 0; index < remainingTargets.length; index++) {
    ...
    const candidates = collectEndpointCandidates(terrain, target, resourceRoadBlockedMask, distanceMap, coreRoadMask)
  ...
  const nextTargets = remainingTargets.filter((_, index) => index !== selectedIndex)
...
function collectEndpointCandidates(
  ...
  forEachCoordinateAtRange(target.coordinate, 1, (x, y) => {
    ...
    const linkCandidates = collectSourceLinkCandidates(terrain, container, resourceRoadBlockedMask, coreRoadMask)
...
function collectSourceLinkCandidates(
  ...
  forEachCoordinateAtRange(container, 1, (x, y) => {
    ...
    if (terrain.get(x, y) === TERRAIN_MASK_WALL || resourceRoadBlockedMask[index] || coreRoadMask[index]) {
      return
    }
  ...
  candidates.sort((left, right) => toRoomIndex(left.x, left.y) - toRoomIndex(right.x, right.y))

  return candidates
}

function areEndpointsReachable(endpoints: readonly ResourceEndpointPlan[], distanceMap: Int32Array): boolean {
  return endpoints.every((endpoint) => hasReachableAdjacentTile(endpoint.container, distanceMap))
}

function hasReachableAdjacentTile(coordinate: RoomCoordinate, distanceMap: Int32Array): boolean {
  let reachable = false
...
function compareEndpointCandidates(left: ResourceEndpointCandidate, right: ResourceEndpointCandidate): number {
  const containerDifference =
    toRoomIndex(left.container.x, left.container.y) - toRoomIndex(right.container.x, right.container.y)
  const leftLinkIndex = left.link ? toRoomIndex(left.link.x, left.link.y) : -1
  const rightLinkIndex = right.link ? toRoomIndex(right.link.x, right.link.y) : -1

  return left.distance - right.distance || containerDifference || leftLinkIndex - rightLinkIndex
}

function reserveEndpoint(endpoint: ResourceEndpointCandidate, resourceRoadBlockedMask: Uint8Array): void {
  resourceRoadBlockedMask[toRoomIndex(endpoint.container.x, endpoint.container.y)] = 1
...
function releaseEndpoint(endpoint: ResourceEndpointCandidate, resourceRoadBlockedMask: Uint8Array): void {
  resourceRoadBlockedMask[toRoomIndex(endpoint.container.x, endpoint.container.y)] = 0
