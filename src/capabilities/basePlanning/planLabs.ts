...
export function planLabs(
  ...
  const labBlockedMask = buildLabBlockedMask(controller, sources, minerals, controllerArea, corePlan, resourceTree)

  const serviceRoadMask = new Uint8Array(ROOM_AREA)

  for (const { x, y } of [...corePlan.roads, ...resourceTree.roads, ...boundaryRoadPlan.roads]) {
    serviceRoadMask[toRoomIndex(x, y)] = 1
  }

  const serviceDistanceMap = buildServiceDistanceMap(terrain, serviceRoadMask, corePlan.roads)

  for (let maxServiceDistance = 0; maxServiceDistance <= MAX_SERVICE_DISTANCE; maxServiceDistance++) {
...
function findLabPlanWithBranch(
  ...
  const searchBranch = (current: RoomCoordinate, remainingLength: number): LabPlan | undefined => {
    if (remainingLength === 0) {
      ...
      const serviceDistanceMap = buildServiceDistanceMap(terrain, serviceRoadMask, roots)
...
function findLabLayout(candidates: readonly LabCandidate[]): LabLayout | undefined {
  for (let firstIndex = 0; firstIndex < candidates.length - 1; firstIndex++) {
    ...
    for (let secondIndex = firstIndex + 1; secondIndex < candidates.length; secondIndex++) {
      ...
      if (!canSupportEightOutputs(firstInput.coordinate, secondInput.coordinate)) {
        continue
      }
...
function canSupportEightOutputs(firstInput: RoomCoordinate, secondInput: RoomCoordinate): boolean {
...
function visualizeLabPlan(plan: LabPlan, visual: RoomVisual): void {
  plan.inputLabs.forEach((coordinate) => visual.structure(coordinate.x, coordinate.y, STRUCTURE_LAB))

  plan.outputLabs.forEach((coordinate) => visual.structure(coordinate.x, coordinate.y, STRUCTURE_LAB))

  plan.serviceRoads.forEach((coordinate) => visual.structure(coordinate.x, coordinate.y, STRUCTURE_ROAD))
}
