...
export function createBaseRegionSelection(
  ...
  const selectedRegionIds = getControllerAdjacentRegionIds(
    controller,
    regionByTile,
  );
  ...
  let numInnerTiles = getSelectedTileCount(
    selectedRegionIds,
    regionTileCounts,
  );

  const addNextRegion = (): boolean => {
    const candidate = getBestMergeCandidate(
      selectedRegionIds,
      regionTileCounts,
      connections,
    );
...
export function selectBaseRegions(
  ...
  return createBaseRegionSelection(controller, regionByTile, regions)
    .selectedRegionIds;
}

function getControllerAdjacentRegionIds(
  controller: StructureController,
  regionByTile: Int16Array,
): Set<number> {
...
function getBestMergeCandidate(
  ...
  for (const candidateId of candidateIds) {
    const candidate: MergeCandidate = {
      ...
      frontierDelta: getMergeFrontierDelta(
        candidateId,
        selectedRegionIds,
        connections,
      ),
    ...
    if (
      ...
      isBetterMergeCandidate(candidate, bestCandidate)
    ) {
...
function isBetterMergeCandidate(
  candidate: MergeCandidate,
  currentBest: MergeCandidate,
): boolean {
...
function getTotalRegionConnections(
  ...
  for (const region of regions) {
    connections.set(
      region.id,
      getAdjacentRegionConnections(region, regionByTile),
    );
  }
